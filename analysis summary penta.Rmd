...
Since it looks like vaccination took off in earnest in May 2018. Looks like there was a drop off in booster in more recent cohorts, suggesting kids aren't getting vaccine on time
...
print("Proportion of all eligible kids registered in KB system who did not or did receive first dose of PENTA, based on month of birth")
print(round(tab1.prop,2))

last.date<-max(monthdates)
...
abline(v=c(last.date-(30.3*9),last.date-(30.3*12),last.date-(30.3*15) ), type='l', lty=2, col='gray')
...
table(year(as.Date(ds1$Measles.Date,'%Y-%m-%d')), ds1$Measles.Given.Denied)

```

## Look at uptake of 1,2, 3 doses by age
First look at  uptake of 1,2, or booster dose by age in all  kids born after March 1,2018. We restrict to this age group This is done as a survival model because of the structure of the data. Many kids in the database don't become age-eligible to receive a dose, so we censor them.
```{r}
...
plot(cov.date$measles.vax.status.date)
...
hist(ds1$age.measles.dose[year(as.Date(ds1$DOB, "%Y-%m-%d"))==2017 ]/30.3, xlim=c(0,18),main='Measles Dose, 9m', breaks=30, freq=F, xlab='Age(m)')
abline(v=9, lty=2, col='red')
hist(ds1$age.measles.dose[year(as.Date(ds1$DOB, "%Y-%m-%d"))==2018 ]/30.3, xlim=c(0,18),main='Measles Dose, 9m', breaks=30, freq=F, xlab='Age(m)')
abline(v=9, lty=2, col='red')

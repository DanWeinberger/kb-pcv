...
## OPV in India
NOTE, SCHEDULE FOR OPV is 6w, 10w, 14w, 16-24m. So timeliness is hard to evaluate here because stop following kids at 24m

```{r import, echo=FALSE}
ds1<-read.csv('../kb_data/KB_Child_Report 2020_3_6.csv')
...
OPV1<-rep(0, times=nrow(ds1))
OPV2<-rep(0, times=nrow(ds1))
OPV3<-rep(0, times=nrow(ds1))
OPV4<-rep(0, times=nrow(ds1))


OPV1[ds1$OPV1.Given.Denied=='Given']<-1 
OPV2[ds1$OPV2.Given.Denied=='Given']<-1 
OPV3[ds1$OPV3.Given.Denied=='Given']<-1 
OPV4[ds1$OPV.Booster.Given.Denied=='Given']<-1 
...
```{r}
hist(as.Date(ds1$OPV1.Date[ds1$OPV1.Given.Denied =='Given']), 'month', main='Date of First dose receipt', xlab='Date')
...
dob.vaccinated.dose1<-as.Date(ds1$DOB[which(OPV1==1)])

hist(dob.vaccinated.dose1, 'months', main='DOB of kids who received 1 dose of vaccine')
dob.vaccinated.dose2<-as.Date(ds1$DOB[which(OPV2==1)])
par(mfrow=c(1,1))
```
...
#table(routine.eligible.flag, OPV1)

#tab1<-table(dob_month[routine.eligible.flag==1], OPV1[routine.eligible.flag==1])
tab1<-table(dob_month, OPV1)
tab1.prop<-prop.table(tab1, margin = 1)

tab2<-table(dob_month, OPV2)
tab2.prop<-prop.table(tab2, margin = 1)

tab3<-table(dob_month, OPV3)
tab3.prop<-prop.table(tab3, margin = 1)

tab4<-table(dob_month, OPV4)
...
print("Number of eligible kids registered in KB system who did not or did receive first dose of OPV, based on Month of birth")
print(tab1)

print("Proportion of eligible kids registered in KB system who did not or did receive first dose of OPV, based on month of birth")
print(round(tab1.prop,2))

plot(monthdates,100*round(tab1.prop,2)[,2], ylab='Percent vaccinated', xlab="Birth Month", main='Uptake by dose and birth month as of Feb 2020', bty='l', ylim=c(0, 100), type='l', xaxt='n', xlim=as.Date(c('2016-07-01','2019-10-01')))
...
table(ds1$OPV1.Given.Denied[routine.eligible.flag==1])
table(ds1$OPV2.Given.Denied[routine.eligible.flag==1])
table(ds1$OPV.Booster.Given.Denied[routine.eligible.flag==1])
...
long1<-ds1[,c('Child.Registration.ID', 'DOB', 'OPV1.Date', 'OPV1.Given.Denied', 'OPV2.Date', 'OPV2.Given.Denied','OPV3.Date','OPV3.Given.Denied','OPV.Booster.Date', 'OPV.Booster.Given.Denied' )]

long1$OPV1.Date<-as.Date(long1$OPV1.Date, format='%Y-%m-%d')
long1$OPV2.Date<-as.Date(long1$OPV2.Date, format='%Y-%m-%d')
long1$OPV3.Date<-as.Date(long1$OPV3.Date, format='%Y-%m-%d')
long1$OPV.Booster.Date<-as.Date(long1$OPV.Booster.Date, format='%Y-%m-%d')
long1$DOB<-as.Date(long1$DOB, format='%Y-%m-%d')

long1$OPV1.Date[long1$OPV1.Given.Denied!='Given']<-NA
long1$OPV2.Date[long1$OPV2.Given.Denied!='Given']<-NA
long1$OPV3.Date[long1$OPV3.Given.Denied!='Given']<-NA
long1$OPV.Booster.Date[long1$OPV.Booster.Given.Denied!='Given']<-NA
long1$censor.date<-max(long1$OPV1.Date, na.rm=TRUE)

long1$OPV1.aged<- as.numeric(long1$OPV1.Date - long1$DOB)
long1$OPV2.aged<- as.numeric(long1$OPV2.Date - long1$DOB)
long1$OPV3.aged<- as.numeric(long1$OPV3.Date - long1$DOB)
long1$OPV.boost.aged<- as.numeric(long1$OPV.Booster.Date - long1$DOB)
long1$censor.age<-as.numeric(long1$censor.date - long1$DOB)
long1$init.age=0
long1<-long1[long1$DOB>=as.Date('2018-03-01'),] #only eligible kids

long2<-long1[,c('Child.Registration.ID', 'OPV1.aged',  'OPV2.aged','OPV3.aged', 'OPV.boost.aged','censor.age','init.age')]
...
dose1.clean<-format.ds(long2.c, 'OPV1.aged')
dose1.clean$dose=1
dose2.clean<-format.ds(long2.c, 'OPV2.aged')
dose2.clean$dose=2
dose3.clean<-format.ds(long2.c, 'OPV3.aged')
dose3.clean$dose=3
dose4.clean<-format.ds(long2.c, 'OPV.boost.aged')
dose4.clean$dose<-4
...
long1<-ds1[,c('Child.Registration.ID', 'DOB', 'OPV1.Date', 'OPV1.Given.Denied', 'OPV2.Date','OPV3.Date','OPV3.Given.Denied', 'OPV2.Given.Denied','OPV.Booster.Date', 'OPV.Booster.Given.Denied','Block' )]
...
long2<-long1[,c('Child.Registration.ID', 'OPV1.aged',  'OPV2.aged','OPV3.aged', 'OPV.boost.aged','censor.age','init.age','Block')]
...
dose1.clean<-lapply(long2.c.spl, format.ds, var1='OPV1.aged', doseN=1)
dose2.clean<-lapply(long2.c.spl, format.ds, var1='OPV2.aged', doseN=2)
dose3.clean<-lapply(long2.c.spl, format.ds, var1='OPV3.aged', doseN=3)
dose4.clean<-lapply(long2.c.spl, format.ds, var1='OPV.boost.aged', doseN=4)
...
ds.cov<-cbind.data.frame('OPV3'=OPV3,'OPV2'=OPV2,'OPV1'=OPV1, 'dose3.date'=as.Date(ds1$OPV3.Date,'%Y-%m-%d'),
                         'dose1.date'=as.Date(ds1$OPV1.Date,'%Y-%m-%d'),
                         'dose2.date'=as.Date(ds1$OPV2.Date,'%Y-%m-%d'),
                         'id'=1:length(OPV3), 'dob'=as.Date(ds1$DOB) )
monthdateseq<-seq.Date(from=as.Date('2018-03-01'),to=as.Date('2020-03-01'), by='month')
...
```{r}
ds1$age.1st.dose<-  as.numeric(as.Date(ds1$OPV1.Date, "%Y-%m-%d") - as.Date(ds1$DOB, "%Y-%m-%d"))
ds1$age.2nd.dose<-  as.numeric(as.Date(ds1$OPV2.Date, "%Y-%m-%d") - as.Date(ds1$DOB, "%Y-%m-%d"))
ds1$age.3rd.dose<-  as.numeric(as.Date(ds1$OPV3.Date, "%Y-%m-%d") - as.Date(ds1$DOB, "%Y-%m-%d"))

hist(ds1$age.1st.dose/30.3)
hist(ds1$age.2nd.dose/30.3)
hist(ds1$age.3rd.dose/30.3)

hist(ds1$age.1st.dose[year(as.Date(ds1$OPV1.Date, "%Y-%m-%d"))==2018 ] /30.3, main='Dose 1 in 2018')

par(mfrow=c(1,3), mar=c(4,2,1,1))
hist(ds1$age.1st.dose[year(as.Date(ds1$OPV1.Date, "%Y-%m-%d"))==2019 ] /30.3, main='Dose1, 6w', xlim=c(0,14), breaks=30, freq=F, xlab='Age(m)')
abline(v=1.5, lty=2, col='red')
hist(ds1$age.2nd.dose[year(as.Date(ds1$OPV2.Date, "%Y-%m-%d"))==2019 ]/30.3, xlim=c(0,14),main='Dose 2, 14w', breaks=30, freq=F, xlab='Age(m)')
abline(v=3.5, lty=2, col='red')
hist(ds1$age.3rd.dose[year(as.Date(ds1$OPV3.Date, "%Y-%m-%d"))==2019 ]/30.3, xlim=c(0,14),main='Dose 3, 9m', breaks=30, freq=F, xlab='Age(m)')

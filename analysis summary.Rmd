...
surv.summary.spl<-split(surv.summary, surv.summary$doseN)
uptake<-sapply(surv.summary.spl, function(x) x$surv[round(x$time)==15])
...
dosemod <- vector("list", length(blocks))
for(i in 1:length(blocks)){
  dosemod[[i]]<- survfit(Surv(dose.comp$agem,dose.comp$doseX)~doseN, data=dose.comp[dose.comp$Block==blocks[i],])
}
...
uptake<-sapply(surv.summary.spl, function(x) (1-x$surv[round(x$time)>15])[1])
uptake

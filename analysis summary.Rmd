...
totalN.sex<-as.matrix(table(ds1$sex))
N.sex.dob.march2018<-as.matrix(table(ds1$sex[as.Date(ds1$DOB)>=as.Date('2018-01-01')] ))
sexnames<-row.names(totalN.sex)
tab1<-cbind.data.frame(totalN.sex,N.sex.dob.march2018)
tab1<-rbind(tab1, apply(tab1,2,sum))
tab1<-cbind.data.frame( c(sexnames,'Total'), tab1)
names(tab1)<-c('Admin. Region', 'All Children', 'Children born after Mar 2018')
row.names(tab1)<-NULL
htmlTable(tab1, rnames=F,align='c',caption="Table 1. Number of children in the Khushi Baby database, October 2019")

long1<-ds1[,c('Child.Registration.ID', 'DOB', 'PCV1.Date', 'PCV1.Given.Denied', 'PCV2.Date', 'PCV2.Given.Denied','PCV.Booster.Date', 'PCV.Booster.Given.Denied','Sex' )]
...
long2<-long1[,c('Child.Registration.ID', 'pcv1.aged',  'pcv2.aged', 'pcv.boost.aged','censor.age','init.age','Sex')]
long2.m<-melt(long2, id.var=c('Child.Registration.ID','Sex'))
long2.c<-dcast(long2.m,Child.Registration.ID+variable+Sex~. )
names(long2.c) <- c('id','variable','sex','aged')
long2.c<-long2.c[!is.na(long2.c$aged),]
...
long2.c.spl<-split(long2.c, long2.c$sex)
dose1.clean<-lapply(long2.c.spl, format.ds, var1='pcv1.aged', doseN=1)
dose2.clean<-lapply(long2.c.spl, format.ds, var1='pcv2.aged', doseN=2)
dose3.clean<-lapply(long2.c.spl, format.ds, var1='pcv.boost.aged', doseN=3)
dose1.clean<-do.call('rbind.data.frame', dose1.clean)
...
dose.comp<-dose.comp[order(dose.comp$sex,dose.comp$doseN),]

dose.comp$sex<-factor(dose.comp$sex)
sexs<-unique(dose.comp$sex)

cols<-c('#e41a1c','#377eb8','#4daf4a','#984ea3','#ff7f00')
dosemod<- survfit(Surv(dose.comp$agem,dose.comp$doseX)~doseN+sex, data=dose.comp)
#ggsurvplot(dosemod, data = dose.comp, pval =F,  fun = "event",keep.data = TRUE, color='sex', palette=cols)

surv.summary<-surv_summary(dosemod,data=dose.comp)
surv.summary.spl<-split(surv.summary, paste(surv.summary$doseN, surv.summary$sex))
uptake<-1-sapply(surv.summary.spl, function(x) x$surv[length(x$surv)])
uptake
#plot(surv.summary$time, 1-surv.summary$surv, col=cols[surv.summary$sex],bty='l', xlab='Age, months', ylab='Proportion vaccinated')
#abline(h=c(0.2,0.4,0.6,0.8), col='lightgray')

surv.summary.c<-1-acast(surv.summary[,c('time','doseN','sex','surv') ], time~doseN~sex )
time.m<-as.numeric(dimnames(surv.summary.c)[[1]])
target.time<-c(1.5,3.5, 9) #when should the vaccine be given?
par(mfrow=c(1,3), mar=c(4,4,1,1))
for(i in 1:3){
...
#dosemod2<- survfit(Surv(dose.comp$agem,dose.comp$doseX)~sex, data=dose.comp)
#ggsurv.list <- ggsurvplot_group_by(dosemod2, dose.comp, group.by = c("doseN"))
```
